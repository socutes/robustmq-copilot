...
export default function SessionList() {
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: 'Created At',
      cell: ({ row }) => new Date(row.original.create_time * 1000).toLocaleString(),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.reconnect_time ? new Date(row.original.reconnect_time * 1000).toLocaleString() : '-',
    },
    {
      accessorKey: 'distinct_time',
      header: 'Disconnect Time',
      cell: ({ row }) =>
        row.original.distinct_time ? new Date(row.original.distinct_time * 1000).toLocaleString() : '-',
    },
  ];
